...
func useHTTPMiddlewares(_ context.Context, e *gin.Engine, injector *wirex.Injector, allowedPrefixes []string) error {
	// Bypass auth and casbin for the internal gateway config sync endpoints and OAuth callbacks
	config.C.Middleware.Auth.SkippedPathPrefixes = append(config.C.Middleware.Auth.SkippedPathPrefixes,
		"/api/v1/gateway/config", "/api/v1/gateway/policies", "/api/v1/gateway/apikeys", "/api/v1/gateway/metrics", "/api/v1/gateway/events", "/api/v1/providers/oauth/callback")
