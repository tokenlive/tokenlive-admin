...
// @Tags PolicyTaggingAPI
// @Security ApiKeyAuth
// @Summary Toggle policy tagging enabled status by ID
// @Param id path string true "unique id"
...
// @Failure 401 {object} util.ResponseResult
// @Failure 500 {object} util.ResponseResult
// @Router /api/v1/policy/policy-taggings/{id}/enabled [put]
func (a *PolicyTagging) UpdateEnabled(c *gin.Context) {
	ctx := c.Request.Context()
	...
	err := a.PolicyTaggingBIZ.ToggleEnabled(ctx, c.Param("id"), item)
	if err != nil {
		util.ResError(c, err)
		return
	}
...
func (a *PolicyTagging) CopyToModel(c *gin.Context) {
	...
	result, err := a.PolicyTaggingBIZ.CopyTemplateToModel(ctx, c.Param("id"), item)
	if err != nil {
		util.ResError(c, err)
		return
	}
