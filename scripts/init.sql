...
(
    ...
    `cached_price`         DECIMAL(10, 6)                                         NOT NULL DEFAULT 1 COMMENT '缓存命中价格（元/百万 Tokens）',
    `cache_creation_price` DECIMAL(10, 6)                                         NOT NULL DEFAULT 3 COMMENT '缓存创建价格（元/百万 Tokens）',
...
(
    ...
    `url`          VARCHAR(1024)                                         NULL COMMENT '供应商 API 基础地址',
    `auth_type`    VARCHAR(64)                                                    DEFAULT 'api_key' COMMENT '认证类型: api_key, oauth_token',
    `api_keys`     JSON                                                  NULL COMMENT '上游API认证密钥列表',
    `oauth`        JSON                                                  NULL COMMENT 'OAuth 凭证(refresh_token/token_endpoint/expires_at)',
    `lock_owner`   VARCHAR(128)                                          NULL     DEFAULT NULL COMMENT 'OAuth 刷新分布式锁持有者实例ID',
    `locked_until` DATETIME                                              NULL     DEFAULT NULL COMMENT 'OAuth 刷新分布式锁过期时间',
    `enabled`      INT                                                   NOT NULL DEFAULT 0 COMMENT '启用状态: 0-未启用，1-启用',
    `description`  VARCHAR(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin         DEFAULT NULL COMMENT '备注描述',
    `creator`      VARCHAR(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin         DEFAULT NULL COMMENT '创建者',
    `modifier`     VARCHAR(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin         DEFAULT NULL COMMENT '修改者',
...
(
    ...
    `url`                  VARCHAR(1024)                                         NOT NULL COMMENT '上游 API 地址',
    `auth_type`            VARCHAR(64)                                                    DEFAULT 'api_key' COMMENT '认证类型: api_key, oauth_token',
    `api_key`              TEXT COMMENT '可选，覆盖 provider 级别的 api_key',
...
(
    ...
    `scope_type`  VARCHAR(32)                                           NOT NULL DEFAULT 'global' COMMENT '适用作用域类型: global/tenant/user等',
    `scope_code`  VARCHAR(128)                                          NOT NULL DEFAULT '' COMMENT '作用域取值(租户编码/用户ID等)',
    ...
    `deleted`     VARCHAR(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL DEFAULT '0' COMMENT '逻辑删除标识',
    `deleted_at`  DATETIME                                              NULL     DEFAULT NULL COMMENT '逻辑删除时间',
    UNIQUE KEY `uniq_policy_tagging_dims_name_deleted` (`model_id`, `name`, `deleted`),
    KEY `idx_policy_tagging_model` (`model_id`),
    KEY `idx_policy_tagging_scope` (`scope_type`, `scope_code`, `deleted`)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
...
(
    ...
    `deleted`     VARCHAR(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL DEFAULT '0' COMMENT '逻辑删除标识',
    `deleted_at`  DATETIME                                              NULL     DEFAULT NULL COMMENT '逻辑删除时间',
    UNIQUE KEY `uniq_policy_loadbalance_dims_name_deleted` (`model_id`, `name`, `deleted`),
    KEY `idx_policy_loadbalance_model` (`model_id`),
    KEY `idx_policy_loadbalance_scope` (`scope_type`, `scope_code`, `deleted`)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
...
(
    ...
    `deleted`         VARCHAR(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL DEFAULT '0' COMMENT '逻辑删除标识',
    `deleted_at`      DATETIME                                              NULL     DEFAULT NULL COMMENT '逻辑删除时间',
    UNIQUE KEY `uniq_policy_invocation_dims_name_deleted` (`model_id`, `name`, `deleted`),
    KEY `idx_policy_invocation_model` (`model_id`),
    KEY `idx_policy_invocation_scope` (`scope_type`, `scope_code`, `deleted`)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
...
(
    ...
    `deleted`         VARCHAR(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL DEFAULT '0' COMMENT '逻辑删除标识',
    `deleted_at`      DATETIME                                              NULL     DEFAULT NULL COMMENT '逻辑删除时间',
    UNIQUE KEY `uniq_policy_limit_dims_name_deleted` (`model_id`, `name`, `deleted`),
    KEY `idx_policy_limit_model` (`model_id`),
    KEY `idx_policy_limit_scope` (`scope_type`, `scope_code`, `deleted`)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
...
(
    ...
    `deleted`     VARCHAR(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL DEFAULT '0' COMMENT '逻辑删除标识',
    `deleted_at`  DATETIME                                              NULL     DEFAULT NULL COMMENT '逻辑删除时间',
    UNIQUE KEY `uniq_policy_route_dims_name_deleted` (`model_id`, `name`, `deleted`),
    KEY `idx_policy_route_model` (`model_id`),
    KEY `idx_policy_route_scope` (`scope_type`, `scope_code`, `deleted`)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
...
(
    ...
    `deleted`                          VARCHAR(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL DEFAULT '0' COMMENT '逻辑删除标识',
    `deleted_at`                       DATETIME                                              NULL     DEFAULT NULL COMMENT '逻辑删除时间',
    UNIQUE KEY `uniq_policy_circuit_break_dims_name_deleted` (`model_id`, `name`, `deleted`),
    KEY `idx_policy_circuit_break_model` (`model_id`),
    KEY `idx_policy_circuit_break_scope` (`scope_type`, `scope_code`, `deleted`)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
