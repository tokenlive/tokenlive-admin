                ...
                <template #bodyCell="{ column, record }">
                    <template v-if="'enabled' === column.key">
                        <a-tag :color="record.enabled === 1 ? 'green' : 'default'">
                            {{
                                record.enabled === 1
                                    ? $t('pages.tagRoute.form.enabled.active')
                                    : $t('pages.tagRoute.form.enabled.inactive')
                            }}
                        </a-tag>
                    </template>
                    ...
                    <template v-if="'action' === column.key">
                        <x-action-button @click="$refs.editDialogRef.handleEdit(record)">
                            <a-tooltip>
                                <template #title> {{ $t('pages.tagRoute.edit') }}</template>
                                <edit-outlined />
                            </a-tooltip>
                        </x-action-button>
...
const columns = [
    ...
    { title: t('pages.tagRoute.form.enabled'), key: 'enabled', width: 110 },
    { title: t('pages.tagRoute.form.creator'), dataIndex: 'creator', width: 120 },
    { title: t('pages.tagRoute.form.description'), dataIndex: 'description', ellipsis: true, width: 200 },
    {
        title: t('pages.tagRoute.form.created_at'),
    ...
    Modal.confirm({
        ...
        onOk: () => {
            return new Promise((resolve, reject) => {
                ;(async () => {
                    try {
                        const { success } = await apis.policy.delRoute(id)
                        if (config('http.code.success') === success) {
                            resolve()
